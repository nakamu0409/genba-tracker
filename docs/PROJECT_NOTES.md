...
- **リグレッションテスト常設済み（2026-07-19〜）**: `npm run test`（vitest）。tests/server/ にリポジトリ層のテストがあり、
  ローカルファイルのlibsql DB＋本物のensureSchemaで実SQLを検証する（getGenbaDbのみモック）。
  リポジトリ層・スキーマ・集計を触ったら必ずテストを実行し、必要ならケースを足す。CIでも実行される。
  注意: libsqlの `:memory:` は接続間でDBを共有できないためファイルDB方式にしている
- サーバーコードで新しいパッケージを直接importしたら package.json の依存に必ず追加する
  （npmはhoistで偶然動くがCIのpnpmでは解決できず落ちる。h3で実際に起きた）
